...
interface CountrySelectorProps {
  ...
  const CountrySelector = ({ onCountrySelect }: CountrySelectorProps) => {
    ...
    return (
        ...
        >
            <SelectTrigger currentValue={value}>
            Select country...
            </SelectTrigger>
            <SelectContent>
            {options.map((type) => (
                <SelectItem key={type.value} value={type.label}><CountryFlag countryCode={type.value} svg style={{ width: '2em', height: '2em', marginRight: '10px' }} />
                <span>{type.label}</span></SelectItem>
            ))}
            </SelectContent>
        </Select>
    );
...
const VehicleSpecs = ({ onNext, onPrevious, dict }: {onPrevious: () => void, onNext: (mode?: string) => void,  dict: SellClassicTranslations | null}) => {
...
<Card className="w-full mx-auto bg-white border-none shadow-none">
  <CardContent className="border shadow-md rounded w-full p-8 space-y-6">
    ...
    <div className="flex items-center space-x-2">
              <Label className='text-lg text-foreground' htmlFor="filter2">Vehicle ID</Label>
        ...
        onChange={(e) => dispatch(setDetails({...store, vehicle_id: e.target.value}))}
                />
            </div>
            <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <Label className='text-lg text-foreground' htmlFor="filter3">Origin</Label>
              <SvgIcon
            ...
            <div className="space-y-2">
    <div className="flex items-center space-x-2">
              <Label className='text-lg text-foreground' htmlFor="filter2">Cubic Capacity</Label>
      ...
      <div className="space-y-2">
    <div className="flex items-center space-x-2">
              <Label className='text-lg text-foreground' htmlFor="filter2">Fuel Consumption</Label>
      ...
      <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <Label className='text-lg text-foreground' htmlFor="filter3">Color</Label>
              <Palette
                width={16}
                height={16}
              />
            ...
            <Select
              name="color"
              value={store?.color}
              onValueChange={(selectorValue) => dispatch(setDetails({...store, color: selectorValue}))}
            >
            ...
            <div className="space-y-2">
            <div className="flex items-center space-x-2">
              <Label className='text-lg text-foreground' htmlFor="filter3">Interior</Label>
              <Palette
              strokeWidth={3}
                absoluteStrokeWidth
                width={16}
            ...
            <Select
              ...
              onValueChange={(selectorValue) => dispatch(setDetails({...store, interior_color: selectorValue}))}
            >
