...
const TipsBlock = ({ tipsBlock }: Props) => {
  ...
  return (
    ...
    <GradientHeading title={tipsBlock?.learnPlatformTitle || "Learn our Platform"} className='mb-9'/>
    <div className='grid grid-cols-3 gap-y-9 gap-x-5 w-full px-4'>
        {tipsData.map((tipsElement) => <ArticleCard tipsDict={tipsBlock ? tipsBlock.tips[tipsElement.ind] : null} tipsInfo = {tipsElement} key={tipsElement.title}></ArticleCard>)}
    </div>
</section>
  );
};
